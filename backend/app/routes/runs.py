...
def get_runs(
    ...
    ).order_by(models.Run.created_at.desc()).offset(skip).limit(limit).all()
...
def get_run(run_id: int, db: Session = Depends(get_db), project: models.Project = Depends(get_current_project)):
...
@router.get("/{run_id}/spans", response_model=list[schemas.SpanResponse])
def get_run_spans(
    run_id: int,
    db: Session = Depends(get_db),
    project: models.Project = Depends(get_current_project)
    ...
    return db.query(models.Span).filter(
        models.Span.run_id == run_id
    ).order_by(models.Span.created_at.asc()).all()
