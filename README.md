...
- **Model** — LLM model used (e.g., "gpt-4", "gpt-4o")
...
|------|----------|---------|
| `llm_call` | LLM inference | ChatGPT, other LLM providers, Ollama calls |
...
```python
with aw.trace(run_name="rag-query", input_text=question, model="gpt-4o") as run:
    # Retrieval
    start = time.time()
...
- LLM-as-judge evaluations for supported models
